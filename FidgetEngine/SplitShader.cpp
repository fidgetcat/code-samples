...
SplitShader::SplitShader(std::string shaderV,std::string shaderF){
    init(shaderV,shaderF);
}

void SplitShader::init(std::string shaderV,std::string shaderF){
...
SplitShader::~SplitShader(){
	...
	glDeleteProgram(shader); //FIX
}

void SplitShader::bind(glm::mat4 tranModel,float r,float g,float b){
    glUseProgram(shader);
    glUniformMatrix4fv(uniformTransform, 1, GL_FALSE, &tranModel[0][0]);
    glUniform3f(uniformColor,r,g,b);
}

void SplitShader::bindCamera(glm::mat4 camera,glm::mat4 tranModel,float r,float g,float b){
    glUseProgram(shader);
...
std::string SplitShader::getFileContent(std::string fileName){
    std::string line;
    std::string output = "";
