...
void SplitShaderTX::init(std::string shaderV,std::string shaderF){
    ...
    uniformMulColor = glGetUniformLocation(shader, "mulcolor");
    uniformAddColor = glGetUniformLocation(shader, "addcolor");
	}
...
void SplitShaderTX::bind(glm::mat4 tranModel,glm::vec3 mulcolor,glm::vec3 addcolor){
    glUseProgram(shader);
    glUniformMatrix4fv(uniformTransform, 1, GL_FALSE, &tranModel[0][0]);
    glUniform3fv(uniformMulColor, 1, &mulcolor[0]);
    glUniform3fv(uniformAddColor, 1, &addcolor[0]);
}

void SplitShaderTX::bindCamera(glm::mat4 camera,glm::mat4 tranModel,glm::vec3 mulcolor,glm::vec3 addcolor){
    glUseProgram(shader);
